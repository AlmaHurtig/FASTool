...
Description:
    This program translate a given DNA sequence to protein sequence and print
    all six possible reading frames.

List of functions:
    'reverse': reverse a given sequence
    'complement': create a complement of the given DNA sequence
...
value.
2. Create a reverse complement of the given DNA sequence and save it to the
variable 'reverse_strand'.
3. Translate the DNA sequence and its reversed complement to protein sequences
by looping through the nucleotides and translate one codon at the time.
4. Return all six frames to the console.

Usage:
./translate_all_six_frames.py DNA_sequence
...
if len(sys.argv) != 2:
    error_msg ='Exactly two input arguments are needed: Usage: ./project_work.py DNA_sequence'
...
def translate_nucleotides(sequence):
    ...
    for i in range(1, len(sequence)-1, 3):
        ...
        if i > len(sequence):
            break
    for i in range(2, len(sequence)-3, 3):
        if i < len(sequence):
...
translated_seq = translate_nucleotides(DNA_seq)
